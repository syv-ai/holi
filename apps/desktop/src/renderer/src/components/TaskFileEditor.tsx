  ...
  return (
    <div data-task-file={task.path} className="flex flex-1 flex-col overflow-y-auto p-4">
      <input
        ...
        className="mb-1 rounded border border-transparent bg-transparent px-1 py-0.5 text-lg font-semibold hover:border-neutral-800 focus:border-neutral-700 focus:outline-none"
      />
      ...
      <p className="mb-3 px-1 font-mono text-[10px] text-neutral-600" title={task.path}>
        {laneLabel(laneOf(task))}
      </p>

      {/* The properties, constrained so they read as a header band, not a field that
          stretches the whole pane. */}
      <div className="flex max-w-md flex-col gap-2 rounded-md border border-neutral-900 bg-neutral-950/60 p-3">
      ...
      <button
        ...
        className="mt-2 self-start rounded px-1 py-0.5 text-xs text-red-400 hover:bg-red-950/50"
      >
        delete task
      </button>

      <div className="mt-3 flex flex-1 flex-col">
        <span className="mb-1 px-1 text-xs text-neutral-500">description</span>
        <TaskDescriptionEditor
          notePath={task.path}
          initial={task.description}
          onChange={onDescription}
          hostClassName="flex-1 min-h-[16rem] overflow-auto rounded border border-neutral-800 bg-neutral-900 text-sm focus-within:border-neutral-700"
        />
      </div>
    </div>
  )
