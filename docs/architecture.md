...
- **The machine's `~/.claude` is not a layer at all.** `CLAUDE_CONFIG_DIR` points a vault agent at **its own directory under `userData/agent-config/`, one per vault** (D86), so the user's global settings, personal skills, plugins, marketplaces and MCP servers are excluded by construction — the agent inherits the vault, not the laptop. Costs one `/login` per vault. Per vault rather than one shared directory is also what makes a vault's **session listing** answer for that vault alone (D100), since `claude agents --json` reads the config directory it is pointed at. See `prd/agent.md` §Config layering.
...
### Per-turn context
A **`UserPromptSubmit` hook** runs on every prompt and emits Holi's fresh context — active note, linked tasks, memory fill indicators, and the vault's sync state. **There is no base system prompt**: `--append-system-prompt` is empty and Holi builds no prompt content, because vault conventions live in `AGENTS.md`, which Claude Code reads natively (`prd/agent.md` §Tool surface). The hook now sources everything from the local filesystem, which removes the transport-and-auth question the old design carried: there is nothing to authenticate to.
...
- **UI system:** a strict **three-layer component hierarchy** — `primitives/` (thin shadcn/Radix wrappers; the ONLY place native `<button|input|select|textarea|dialog>` or Radix may appear) → `composites/` (domain-agnostic patterns) → `features/*` (domain-aware). Lower may never import upper; a feature may not import another feature. Built on **shadcn/Radix + Tailwind v4 CSS-first tokens** (`index.css` speaks shadcn's full semantic vocabulary + a `--radius` scale + the **motion vocabulary** described below). An ESLint gate (`eslint-plugin-boundaries` + `no-restricted-syntax`, via a lint-staged pre-commit hook) enforces the layering, bans native elements outside primitives, bans cross-feature/upward imports, bans native `title=` tooltips, bans arbitrary colour literals (tokens or nothing), and bans motion numbers stated at a call site (D98).
  ...
  - **Motion is four named behaviours from one vocabulary** (2026-09-13, **D98**). `index.css` §Motion tier defines two curves and five durations plus a stagger step, and **every animation in the app names which of four behaviours it is — anything that cannot name one does not animate**. **R · respond**: under your pointer or caret and nowhere else, always a *transition* so it reverses the instant you leave and can never be in your way. **A · arrive**: something enters or leaves the layout, from the direction it belongs to, and leaving is faster than arriving because a thing on its way out is in your way; an *animation* rather than a transition, because Radix's `Presence` keeps an exiting node mounted by waiting for `animationend`. **K · acknowledge**: one beat, once, for a discrete act you committed; never blocks, never queues, and a repeat replays (`lib/use-ack.ts`, which forces a reflow because removing and re-adding a class in one tick does not restart an animation). **F · in flight**: the only looping motion, bound to a real in-flight state, so when the state ends the motion ends and nothing loops decoratively. **Weight without overshoot** — `--ease-spring` and the `pop`/`wake` keyframes live in `features/onboarding/onboarding-ritual.css` and nowhere else, because a one-time ceremony is allowed one spring and the daily app is not. Tokens are **purpose-named, not size-named**, so re-pacing the app is an edit to one block. **Inside a document the line is "is it a thing you can click"**, not "is it in the editor" — see [`prd/notes-editor.md`](prd/notes-editor.md)'s callout — and everything there is paint only. Arrivals are **computed, never declared** (`arrivalIndex`/`useArrivals`): an animation declared on a list's children replays on every re-render. Design: [`specs/2026-09-12-motion-system-design.md`](specs/2026-09-12-motion-system-design.md).
  - **Every sidebar is one drawer** (2026-09-26). The nav, history, the last turn and the PDF viewer's sidebars share speed, form and layout, and the three that are Holi's own DOM are one component, `composites/DrawerShell.tsx`. A drawer **pushes**: its column's width runs 0 → w and back at the slide pace (`--motion-slide`, `--ease-slide`, the same both ways), moving what is beside it, as a *transition* so a press mid-slide reverses it, with the content held at w and anchored to the far edge so it travels in whole. That makes it the one place outside CodeMirror where a layout property animates, and the editor beside a drawer reflows during the slide: the accepted cost of pushing rather than covering. It owns its column rather than living in `react-resizable-panels`, which cannot animate a size. Every drawer opens at 320px and resizes between 150 and 560 (`lib/drawer.ts`), by pointer (1:1, never eased) or arrow keys, each remembering its own width; its header is `PanelHeader`'s 44px row, and its inner edge and header rule are the vault theme token `drawer-edge`, clear by default. The nav stays mounted while hidden (⌥⌘S, `nav.toggle`, or the hide button in its header), so the tree keeps its state, and hidden it leaves a 44px rail at the left edge: the show button, then an orb per running session drawn by the chats section's own status rule (`useSessionIndicator`), a press opening it, then the apps as a menu. The nav is the only place a session or an app can be clicked, and the orbs are where you see which session needs you; the others unmount once slid out. The PDF sidebars are the PDF library's DOM, so they copy width, motion, header and edge through the stylesheet Holi injects (`lib/pdf-viewer-config.ts`), and are the one exception to resizing: the library offers no way to, and adding a handle would mean writing into its layout.
...
- **A binary the renderer renders itself arrives as bytes over the seam** (D103, 2026-09-21): `files.read` returns a `Uint8Array`, structured clone carries it, and the renderer builds a blob URL. `holi-vault://` stays the route for `<img>` only; §10 has why it gets no CORS header. **A third-party UI inside a shadow root is themed by a token map, not restyled**: the PDF viewer's palette is fed `var(--token)` strings from `lib/pdf-viewer-config.ts`, which cross the shadow boundary and resolve against the vault's theme, so D64 holds there with no colour literal and no CSS injection.
- **App shell:** single-window, atom-driven view model, drawers/dialogs as summoned modals. **Every app-level action is one row of one table** (`state/commands.ts`, D102, 2026-09-21): the keydown dispatcher, the command palette's `>` list and the application menu all run a command by id through it, so a shortcut hint is never a second list. The palette itself is [`prd/command-palette.md`](prd/command-palette.md). **One forward-looking constraint:** the pane/tab system must not assume tabs are notes — post-v1 **vault apps** ([`prd/vault-apps.md`](prd/vault-apps.md)) open as first-class app tabs. Design in [`prd/notes-editor.md`](prd/notes-editor.md) §Panes & tabs.
...
- **`holi-vault://` answers `<img>` and nothing else, on purpose.** A renderer `fetch` of it fails on CORS because its responses carry no `Access-Control-Allow-Origin`, and adding one would open the protocol to the sandboxed vault-app frames: a packaged `file://` renderer sends the same `null` origin they do, so no origin check can tell the two apart, and D74's isolation rests on that header's absence. A renderer that needs a file's bytes asks main for them over the seam (§9, D103).
