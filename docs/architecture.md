...
### MCP ops
A **local MCP server in Electron main** exposes only the non-file ops — in v1 just **tasks** and **`note_rename`** (calendar/mail join in phase 2). Per-run bearer token (as before); ops proxy to the Syv tRPC API. Everything else is Claude's native `Read/Write/Edit/Bash/Glob/Grep` on working copies — any vault action that's "just a file op" needs no custom tool.
...
Structured records on the server, not files — tasks are inherently structured, the agent already manipulates them via ops, and file-based tasks were the source of the old complexity (the derived `area` phantom, orphan rescue, `source_file`-as-home). Server records make shared boards and real-time updates trivial. Deep rationale: [`prd/tasks.md`](prd/tasks.md). Shape (`packages/shared`):
...
- **Agent:** manipulates tasks via MCP task ops (create/list/set/complete/link), membership-gated.
