...
2. **Whitelist = colours + paint-only chrome.** The 19 semantic colour tokens (`background`/`foreground`/`card`/`popover`/`primary`/`secondary`/`muted`/`accent`/`destructive` + `-foreground` pairs, `border`, `input`, `ring`), plus `scrollbar-thumb(-hover)` and `selection`; and chrome: `radius`, `shadow-popover`, `shadow-dialog`. Every knob is paint-only. Titlebar/window chrome was **excluded** — see Deferred.
   ...
   **`scrollbar-thumb` reaches further than it did (2026-08-20).** It was consumed by a `.holi-scroll` class that three elements wore, so a vault that recoloured it recoloured three scrollbars and Chromium's light-grey default painted the other twenty-five. The rule is global now — a bare `::-webkit-scrollbar` selector, which reaches the CodeMirror scroller, the terminal viewport and a Radix menu alike — so the token means what the whitelist always said it meant. `color-scheme` is also stamped per theme, which is not a vault knob: it follows `data-theme`, and it is what stops everything Holi does not paint itself from defaulting to light.
   - **`brand` joined later (2026-08-20), and the reason generalizes.** Recorded as an amendment under D64 rather than as its own number: it widens what a token *reaches*, not what a theme is allowed to *do*, and D64's guarantee — a vocabulary containing nothing that can move, size or re-space anything — is untouched by it. `--primary` is the brand as a **fill** — it is paired with `--primary-foreground` on top of it — and on a dark-first theme a fill dark enough to carry near-white text is far too dark to *be* text on a near-black background: sky-700 on neutral-950 measures **3.38:1**, under AA. `--brand` is the same brand as **text** (sky-400 dark / sky-700 light, **9.09:1**), and every `text-primary` in the tree moved onto it. A vault recolouring the brand should set both; setting only `primary` recolours the fills and leaves the text where it was. The same split is handed to vault apps (`APP_BASE_TOKENS`) and taught in the authoring skill, because the worst instances of the old blue were an app's own links.
   - **`divider` joined the same way (2026-08-20), and for the same shape of reason.** A third amendment under D64, again widening reach rather than power. `--border` was doing two jobs: the edge of an **object** (a card, a chip, a popover) and the hairline between two pieces of **chrome** (the split between panes, the sidebar's edge, a panel header's underline, the footer's rule). They want opposite weights — an object's border has to be *seen*, because it is what says where the object ends; a chrome rule only has to be *noticed*, and at `--border`'s weight a window full of panes reads as a grid of drawn boxes rather than one surface with seams in it. `--divider` takes the chrome half (18 sites, including the four that had hand-rolled it as `border-border/50`). Its default is **derived, not stated** — `color-mix(in srgb, var(--border) 55%, var(--background))` — so a vault that recolours `border` or `background` gets a matching divider without knowing this token exists; a vault that wants an outright colour sets it, as a flat colour, since the validator's `isColorLike` accepts hex/`rgb`/`hsl`/`oklch`/keywords and not `color-mix`. One definition covers both modes: the light block re-states `--border` and `--background` on the same element, so the substitution resolves per theme.
