...
| Scopes | ~~Read-only both~~ **superseded by D68 (2026-08-05), amended by D69**: mail is `gmail.modify` + `contacts.readonly` + `contacts.other.readonly`; calendar stays `calendar.readonly`. Compose = `mailto:`/deeplink |
...
- ~~**Scopes (v1, read-only):**~~ **Superseded by D68, 2026-08-05.** As shipped: `gmail.modify`, `calendar.readonly`, `contacts.readonly`, `contacts.other.readonly`, plus `openid email` for identity. No calendar write. `gmail.modify` replaced `gmail.readonly` (a superset) to buy mark-read, star, archive and trash; `contacts.readonly` rode the same consent screen to put a real address book behind `@`-completion.
  - **Contacts is two scopes (D69).** `contacts.readonly` covers `people/me/connections` — contacts explicitly saved, which for a Workspace account is often almost nobody. The auto-collected ones live at `otherContacts` and need `contacts.other.readonly`; that collection also takes **`readMask`** rather than `connections.list`'s `personFields`. Both mismatches are 4xx that `listContacts` swallows by design, so the symptom of getting either wrong is an address book that answers successfully and is empty.
...
2. **Remote content — ours, and the half that is easy to forget.** DOMPurify stops a script; it does nothing about `<img src="https://tracker/pixel.gif">`, which is a read receipt fired at the sender from the user's IP the moment the message is opened. So `src`/`poster` are stripped and **counted**, `srcset`/`background` are forbidden outright, and `url()` inside an inline style is dropped declaration-by-declaration (the surrounding colours and spacing survive). The count is what drives **"Load images"**, which re-sanitizes the original HTML with the flag flipped — one code path, never a re-injection of stashed URLs.
   - **Two permissions with two lifetimes (D69).** "Load images" is *this message*, remembered for the session; "Always from this sender" is persisted per address in `userData` and shown, with a way to forget them, in settings. The per-message choice used to live in the component, and the reader unmounts on every thread close — so it did not survive closing a thread and coming back. `data:` images are kept: nothing is fetched, so nothing is disclosed. `cid:` and relative sources are dropped silently, because "load images" cannot fix them.
...
**Two canvases, not one (D69).** The frame originally always took the app's theme. Real mail declares its ink and inherits its paper, so a dark theme rendered a great deal of it black-on-black — and its images with it, a logo usually being dark ink on transparency. `bringsOwnDesign` reads the **raw** HTML for any colour, `bgcolor`, `<font>` or `<img>`; anything at all means a white canvas with `color-scheme: light`, and only a message that brings no design of its own inherits the theme. Deliberately asymmetric: wrongly giving paper costs a white block in a dark app, wrongly giving the theme costs an unreadable message.
...
- **The cache key names every filter that narrows the list (D69).** It is `query|category|unread`. It was `query|category` while `unread` also composed into the Gmail query, so the two lists shared one entry — a warm cache served the whole inbox for "unread only", and a cold unread-only fetch wrote its narrower answer under the plain inbox's key. The failure of an under-specified key is a confidently *wrong* list, not a stale one.
- **A thread can leave a list without being deleted (D69).** Archive is `labelsRemoved: ['INBOX']` and trash is `labelsAdded: ['TRASH']`; neither is `messagesDeleted`, which was the only departure `merge` understood. The refetched summary — `threads.get` still answers, the thread is in All Mail — was therefore re-added as *new mail at the top of the list it had just been archived out of*, and stuck, because the merged list was written back and the cursor advanced past the event. `readHistory` now reports departures separately, `merge` applies them to the added branch as well as the kept one, and an explicit search degrades to a refetch because a search may still legitimately match an archived thread.
