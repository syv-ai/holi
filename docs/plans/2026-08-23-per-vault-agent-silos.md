...
- ~~**A malformed `.claude.json` reads as logged in.**~~ **Overturned by verification in the running app (2026-08-23).** Reading `oauthAccount` at all is wrong: a session printed `Not logged in` in a directory whose `.claude.json` carried one. The key records an account, not whether the credential behind it is reachable — that is a keychain entry, which can be locked or re-keyed without the file changing. Replaced by `takeFirstSpawn`, a `.holi-spawned` marker Holi writes into the config directory — a proxy rather than a guess, since credentials are keyed to the directory.
...
- **`isAgentSignedIn` was a false positive where it mattered.** The migrated vault reported `oauthAccount` in `.claude.json` while its session printed a bare `Not logged in`, so the notice was suppressed at the one moment it was wanted. Replaced by the `.holi-spawned` first-spawn marker, which is Holi's own state and cannot disagree with itself.
- **A claim in this plan was wrong and is withdrawn.** It said the migration "keeps its login". The credential is not in the directory — it is a keychain entry Claude Code owns — so that was never Holi's to promise. What the rename carries is files: transcripts and the plugin set. On re-test the login *was* working at the renamed path, and the earlier `Not logged in` is best explained by a locked login keychain (`security dump-keychain`, run to investigate it, unlocks one). Recorded rather than asserted either way.
