...
Soft turn-taking makes human-vs-agent same-region collisions rare; when they do overlap, character-level last-writer resolves it (same as two Google-Docs cursors), with the safety net below. **Staleness** within a turn is handled by Claude Code itself: `Edit`/`Write` require a prior `Read` and fail if the file changed since (D2). The bridge is the **only genuinely novel component left in the plan → Spike 1**, before any other code (D25).
