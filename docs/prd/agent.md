...
- **`holi`**: Holi's own generated script, the same shape, on the same loopback hook server and per-session token as the turn signals. It asks the app to do something (`app open`, `app init`, `seed refresh`) or reads what only the app can parse: **`holi pdf comments <path> [--json]`** (D106) prints a vault PDF's comment threads, read through PDFium in main ([`pdf-export.md`](pdf-export.md) §Viewing a PDF). That one is read-only, so the seeded `.claude/settings.json` allows it outright (`permissions.allow`, merged into existing vaults like `ask`), and the managed `pdf-comments` skill says when to use it. It prints text rather than JSON by default and exits non-zero on a refusal, because its output is the answer itself.
