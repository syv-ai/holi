...
- The client keeps **no local vault registry**: the vault list is a server query — refetched live when you **join or leave** a vault, so a vault you are invited to appears in the switcher without a restart, and one you are removed from disappears.
  - **This needs a channel scoped to *you*, not to a vault**, and is the reason the SSE stream is per signed-in user rather than per vault (`architecture.md` §5). "You were added to a vault" is not about any vault you are already listening to, so under a per-vault stream it had **nowhere to arrive** and the switcher stayed stale until app restart. A stream that resolved your vaults once at connect would be wrong from the same moment, so the `membership` frame also **re-keys the live connection** — that, not the refetch, is the load-bearing part.
  - Refetch rather than a pushed `Vault`: on `left` you can no longer read the vault, so there would be nothing to hand you. The list is one indexed query.
...
- Tasks are structured server records **projected as files** ([`tasks.md`](tasks.md)): they live at `tasks/<slug>-<id>.md`, not in the old `.holi/tasks/` directory, and nothing derives task state by scanning them — the board, agenda and reminders all read the record. No task-count-per-file badges derived from wiki-links.
...
- **Restore:** a user can browse the timeline, **read a version**, and restore it. Restore is a server-side operation that produces a new CRDT state (not a destructive rewind of shared truth), takes a `pre-restore` snapshot first, and — because `replaceAllText` goes through the live room — is an edit every connected member sees, not a private undo. **Shipped 2026-07-16** as a drawer beside the editor.
  - **Reading a version before restoring it is part of the feature, not a nicety.** `snapshots.list` deliberately never returns snapshot *content* (a full Y.Doc per row), so a timeline built on it alone would offer "restore 10:31" versus "restore 09:58" and no way to tell them apart — automatic snapshots carry no label, so the only thing on screen would be a timestamp. A `snapshots.preview` procedure returns the text; it is restore with the write removed.
  - **Retention is still open — see Open questions — and the timeline does not answer it.** A doc edited all day mints a snapshot every 10 minutes, so within a week the timeline is mostly automatic rows and the "before Claude edited" snapshot the feature exists to surface is buried. That is a *rendering* problem and it has a rendering fix: milestones (an agent write, a rename, a restore) are listed, and automatic snapshots fold behind a "show all versions" toggle. **Nothing is deleted.** `yjs_snapshots` still holds a full Y.Doc per row and still grows unbounded; pruning is data loss and needs its own decision.
...
- **Exporter:** debounced continuous commits of vault content to the **default branch**. Exported set: docs + **tasks** + `.claude/**` + `.holi/settings.json`. The exporter has **two sources** — the `docs` ⋈ `yjs_docs` join and the `tasks` table.
- **Tasks *do* appear in the repo** (revised 2026-07-14). This **reverses** the earlier v1 position ("tasks never appear in the repo; remote sessions can't see or edit tasks in v1, accepted"), and with it the rejection of *a task-file round-trip* on the grounds that it re-opens file↔record complexity. It was built instead, because [`tasks.md`](tasks.md) made tasks a writable **file projection** for the *local* agent anyway — so the round-trip machinery (a parser, a serializer, a per-field patch, a conflict rule) already existed and had to exist. Extending it to git was **additive**, not a new seam: the exporter gained a second source, and the ingester gained a branch that routes `tasks/**.md` to the record path. The payoff is that a remote Claude Code session reads, edits, creates and deletes tasks exactly as the local one does.
- **Ingester:** webhook-driven. Foreign commits on the **default branch only** are ingested by diffing each changed file against the last-exported base and applying the patch as **positioned Yjs ops** — the same spike-proven frozen-base diff-merge shape as the bridge turn protocol, run server-side, guarded by pre-ingest snapshots. When the live doc has diverged from the export base, the merge still applies (Yjs convergence, both texts survive) and a `diverged-ingest` **warning** is recorded on the vault's git status — the non-blocking *"let Claude reconcile?"* overlap flag takes over when the merge-safety-net feature ships.
...
- **Edge policy:** binaries ignored (the vault is text by construction); renames ingest as identity-preserving path moves with no auto link-rewrite; a force-push pauses the mirror until the owner resolves; every ingested path goes through `VaultPath`. Ingest-driven doc deletes take **no pre-delete snapshot** — snapshots cascade with the doc row, the same recovery gap as any doc delete today.
