...
- **A per-file timeline** — a right-hand drawer, opened by a header button on a note or by the `v.N` at the end of the note's frontmatter header. Its own header counts the file's revisions at the right ("45 revisions"), the same uncapped count as that `v.N`, from `notes.fileHistory`, rather than the rows it lists, which stop at 200. It is a drawer like every sidebar in Holi (`DrawerShell`, [`../architecture.md`](../architecture.md) §9): it pushes in from the right and back, and resizes. A **flat** `git log --follow` for the open file: every commit, newest-first, each showing its short sha, message and author. Selecting a commit shows, inline, the diff that commit made to *this* file (vs its parent) in a read-only [`@codemirror/merge`](https://github.com/codemirror/merge) view; the sha opens the commit on GitHub; **Restore** writes that version's content back as a new commit.
