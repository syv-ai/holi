...
- **A per-file timeline** — a right-hand drawer, opened by a header button on a note or by the `v.N` at the end of the note's frontmatter header. Its own header counts the file's revisions at the right ("45 revisions"), the same uncapped count as that `v.N`, from `notes.fileHistory`, rather than the rows it lists, which stop at 200. It opens and closes with its content sliding at the slide pace (`--motion-slide`, eased in and out, the same both ways); the close is held until the slide ends, the split-pane exit pattern, and a press during it reopens instead. The panel's width does not animate: that would fight the resizable group's sizing and make the editor beside it reflow every frame. A **flat** `git log --follow` for the open file: every commit, newest-first, each showing its short sha, message and author. Selecting a commit shows, inline, the diff that commit made to *this* file (vs its parent) in a read-only [`@codemirror/merge`](https://github.com/codemirror/merge) view; the sha opens the commit on GitHub; **Restore** writes that version's content back as a new commit.
...
- **A conflict in `.holi/settings/app.yaml` or `.claude/settings.json`** is a conflict in config, not content, and can leave the vault misconfigured while it lasts. The reconcile path handles it like any file, but these are the ones worth showing prominently.

### `.holi/settings/app.yaml` has a schema (D85)
...
**The resolver's `warnings[]` are shown**, each beside the setting it names, which nothing displayed before — a malformed value was replaced by a default and never mentioned. And the files stay the interface: the pane offers `.holi/settings/app.yaml`, its `.local` sibling and `.holi/settings/theme.css` as tabs, the same escape hatch "Edit Icon…" gives its map.
