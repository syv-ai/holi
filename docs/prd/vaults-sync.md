...
9. **FR-9** A **`pre-commit` hook runs four vault-wide transforms** at the commit boundary — `relink` (rewrite `[[links]]` for a file moved outside Holi), `archive-done` (off by default), `scaffold-md` and `normalize-md`. They rewrite and **restage**, so the fix is in the commit being made rather than the next one.
