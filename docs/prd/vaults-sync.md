...
18. **FR-18** **Ask Claude to reconcile** (a) pauses autosave and auto-pull, (b) re-runs the merge so the conflict is really in the tree, (c) opens the agent drawer seeded with a prompt naming the conflicted paths, (d) resumes normal operation once the tree is clean. Built ([`agent.md`](agent.md) §The agent as merge resolver names the parts). (a) needed no code of its own: a merge in progress is already a `blockedReason`, so (b) is what stops the loops and the agent's merge commit is what releases them.
19. **FR-19** During a reconcile, the conflicted files are **read-only in the editor**. Everything else in the vault stays editable. **Not built** ([`../not-built.md`](../not-built.md)).
20. **FR-20** The user can always **abandon** a reconcile: `git merge --abort` returns to the pre-merge state, which is a commit, so nothing is at risk. True of the repo, and **not offered in the app**: `main/git.ts` has `abortMerge`, and the only thing that calls it is the auto-pull's own conflict path — no button, no procedure ([`../not-built.md`](../not-built.md)).
