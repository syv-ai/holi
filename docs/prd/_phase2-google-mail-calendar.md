...
> Deferred to **Phase 2** (after PDF/docx) per **D17**. Stub only.
...
## Summary
Let employees sync their **@syv.ai Gmail and Google Calendar** into Holi — read their mail/threads and calendar events, and link them to tasks and notes. Because identity is already **Google Workspace SSO** (D7), this rides the same OAuth: no separate mail client, no bridge, no handshake.
...
- Agent MCP ops for mail + calendar (the two of the ~5 surviving ops that are external services — see agent PRD / D10).
