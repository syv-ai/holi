...
**Still unproven, and to be treated as broken:** everything *outbound* — `buildRfc822`, `messages.send`, `drafts.create` and reply threading — plus `postJson`'s unreadable-2xx path, the three `events.*` writes, and whether the agent's send gate fires on the command text it actually emits. **The inbound half of this pillar is proven; the gap is exactly the half that reaches another human.**
