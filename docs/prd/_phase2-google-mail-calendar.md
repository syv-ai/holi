...
**What has actually run, as of 2026-08-14.** Proven in real use: **mail read** end to end — loopback + PKCE, token refresh, the thread list, opening a thread, the sanitised reader, search, categories and the `history.list` delta — the **four triage writes** (mark read, star, archive, trash), and the **calendar agenda**. That is a bigger claim than it looks: triage working means `GoogleApi.post`, `modifyThread`, the trash endpoint and `googleData`'s Google-first write ordering are all real rather than assumed.

**Still unproven, and to be treated as broken:** everything *outbound* — `buildRfc822`, `messages.send`, `drafts.create` and reply threading — plus `postJson`'s unreadable-2xx path, the three `events.*` writes, whether the agent's send gate fires on the command text it actually emits, and the **People contacts** call behind recipient autocomplete. That last one deserves its own sentence: `listContacts` never rejects and caches `[]` on refusal, so a scope or `readMask` failure there is indistinguishable from an empty address book. It is the one surface that can be broken without ever looking broken.

*This paragraph replaces a blanket "nothing in this repo has ever talked to Google", which was true when it was written and had survived into four documents after it stopped being true — the pillar's own failure mode applied to its own prose.*
