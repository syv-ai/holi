...
> **Nothing in this document describes code that exists.** It is a design held for later, and it is listed in [`../not-built.md`](../not-built.md) rather than among the PRDs, which describe the product that ships. Its open questions are open questions about an unbuilt thing, which is why they stay here rather than folding into a specification.
