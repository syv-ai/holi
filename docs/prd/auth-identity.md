...
- **`USER.local.md` or `.holi/settings.local.json` committed by accident** — the seed `.gitignore` prevents it, but an *adopted* repo (or one created before the seed changed) would lack the entries. So the check is active and runs on **every vault open**, not at creation: `gitignoreWithLocalOnly` merges the required lines **line-wise**, leaving an adopted repo's own ignores alone and doing nothing at all when they are already there. It does not ask, because there is no version of this the user would decline, and it replaces a boundary the server used to enforce structurally.
