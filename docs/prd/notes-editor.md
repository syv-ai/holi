...
13. **File tree & folders.** Driven by the **filesystem** — the tree is a directory walk plus a watcher. Create/rename/move/delete through ordinary file operations.
    ...
    - **A note may name its own icon**, as `icon: 🎯` in its frontmatter, and that emoji replaces the type glyph in the tree. The emoji lives **in the note** rather than in a per-vault map keyed by path, because a path-keyed map rots: notes here are moved by things that never go through Holi — the agent's `mv`, a terminal, a pull — and every one of those would detach an icon from its note and leave a dead entry behind, with nothing to notice. Frontmatter travels with the file, so a move costs no migration code at any of the call sites that move one. It is the same reasoning that already keeps `kind` in the file's own frontmatter rather than in its filename, and so it reads the **leading block only**, exactly as the daily-note check does — a note containing a horizontal rule and an `icon:` line is a note *about* icons, not a note *with* one. The value must be **exactly one emoji** (`RGI_Emoji`, so a ZWJ sequence, a skin tone, a flag and a keycap each count as one); anything else is ignored rather than truncated, and the note keeps its type glyph. **Both spellings are accepted**, because emoji split in two and RGI holds only one form of each: a character that is emoji-presentation by default (⭐, ✅) is in the set bare and *not* with a variation selector, while one that is text-presentation by default (❤️, ▶️) is in the set only *with* it — and the macOS picker emits the selector for both. Testing the literal value alone therefore refused what the palette produced, so the value is also tried with its selectors stripped; that cannot loosen anything, since what remains still has to match RGI on its own. A lone **pictograph written without its selector** (`❤`, `⚙`) is accepted too: RGI is strictly right that the bare character is not an emoji, but it is one to whoever typed it — copying a symbol out of a web page produces exactly that form — and refusing it in silence is the same failure. `Extended_Pictographic` is the widening, and it holds no letter, digit or punctuation, so an arrow, a circled digit and a CJK character all stay refused. The emoji is returned exactly as the file spells it and never re-normalized. **The name is not the carrier.** A leading emoji in the *filename* was the first reading of the request and is the cheaper build — pure projection, and it would cover folders too — but it puts the emoji in the path, and then a wiki-link can only be written by someone who remembers which icon a note wears. **`.holi/icons.json` is the second home**, for everything frontmatter cannot reach: **folders**, non-markdown files (a PDF, an image), and the agent-surface files — `CLAUDE.md` and `AGENTS.md` are read verbatim as the agent's instructions, so frontmatter there would become prompt text, and local-only markdown like `USER.local.md` never reaches `docs` at all (the scan files it under `files`, and only a `DocMeta` carries the field). Its layering is the theme's (D64): `.holi/icons.json` is committed and shared, and a gitignored `.holi/icons.local.json` overrides it **per key**. Keys are ordinary vault paths, normalized like every other (`Clients/` and `Clients` are one folder), and a key reaching outside the vault is dropped rather than resolved. **A note's own frontmatter outranks the map**, so the note stays the owner of its icon and the map is the fallback. A path-keyed map can rot — that is exactly why it was refused for notes — but an entry whose file moved degrades to *no icon*, never to lost content, and refusing to let anyone icon a folder to avoid a cosmetic staleness is the worse trade.
    - **"Edit Icon…" in the row menu** writes that map, for any row: a note, a folder, a PDF. One gesture with one destination is what makes it explicable, and the map is the only home a folder or a binary has. An empty field clears the entry rather than writing an empty one. When the note's own frontmatter names an icon the dialog **says so**, because writing an entry that is then silently outranked is the one outcome that would make the gesture look broken.
