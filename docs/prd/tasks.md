...
- **`due` may name an hour too**, and the absence of one is meaningful: a task due `2026-08-25` is due that day and goes overdue when the day has passed; one due `2026-08-25T14:00` goes overdue at 14:01. Both fields are edited with the same picker (`composites/DateTimePicker`), which is what makes them read as one system. **The month grid is one tab stop with a roving focus inside it**: arrows walk a day or a week, `PageUp`/`PageDown` a month (clamped, so 31 March back a month is 28 February — `nextDue`'s rule, for the same reason), and a walk that leaves the month pages the view rather than stopping at the edge. The grid claims only the keys it moves on, so `Tab` and `Escape` still leave the popover — a date grid that traps focus is worse than one you cannot navigate. Design of record: [`../specs/2026-08-21-task-datetime-design.md`](../specs/2026-08-21-task-datetime-design.md).
