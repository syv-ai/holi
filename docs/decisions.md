...
1. ~~**Mail is extracted as plain text, never sanitized HTML.**~~ **Overruled by Nicolai the same day, and the original design (sanitize) stands.** The build had argued that extracting text removes the XSS/tracking class *by construction* and adds no dependency. The reason that was wrong: **a mail reader that cannot render mail is not a mail reader** — the cost ("designed newsletters read plainly") was the product, not a footnote, and "we removed the feature so we would not have to secure it" is not a security win. What shipped instead:
   ...
   - **`<style>` blocks dropped** — a message's stylesheet has no scope, so `body{display:none}` in a newsletter would restyle the app around it. Inline `style` attributes survive, which is how mail is actually designed.
   ...
   - Links are opened with `window.holi.openExternal` from a delegated click handler, never navigated — `target` is stripped, and the handler `preventDefault`s before deciding, so an untrusted scheme still cannot navigate.
   - A **narrow renderer CSP** was added alongside (`object-src`/`frame-src`/`child-src`/`base-uri`/`form-action` all `'none'`), closing a standing gap: there was no CSP at all. It is deliberately *not* an XSS defence — `script-src` is absent because Vite's dev preamble is inline, and a policy carrying `'unsafe-inline'` reads like protection while stopping nothing.
