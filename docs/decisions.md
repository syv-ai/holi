...
- **Version history over `git log` (also-settled) — landed (2026-07-31).** A `git show`/`changedFiles` seam under a `history` router (`list`/`log`/`changed`/`fileDiff`/`restore`); `state/history.ts` is rewritten off the deleted `snapshots` API onto commits keyed by **path**; a per-file drawer (header button) and a whole-vault commit dialog (from the footer sync state) render commits and inline `@codemirror/merge` diffs, sha-linked to GitHub. Restore writes the old content as a new commit, never a history rewrite. Plan: `plans/2026-07-28-version-history.md`.
