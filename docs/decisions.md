...
**Not verified, and not verifiable here:** nothing in this pillar has ever run against Google — the live OAuth round-trip, agenda, mail, and the agent CLI all answer against fakes in the suite. The remaining gate is whether the registration is **Desktop-type** (or carries the `http://127.0.0.1` redirect). The mail frame is likewise pinned only in jsdom, which is not Chromium: no packaged build has rendered a real message.
