...
- **Rule:** the bridge must **diff**, never blind-replace the CRDT. A full-document `Write` becomes a diff-merge; non-overlapping edits merge cleanly, and overlapping simultaneous edits to the same characters converge with **both texts surviving side by side** (concurrent deletes merge; concurrent inserts land adjacently in deterministic clientID order — *spike-verified*, see [spikes/2026-07-10-bridge-turn-protocol.md](spikes/2026-07-10-bridge-turn-protocol.md); the D26 overlap flag covers the garble). *(The full turn protocol — frozen base + soft turn-taking — is **D25**.)*
