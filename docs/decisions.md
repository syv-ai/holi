...
7. **External writes reconcile in the editor**: clean buffer reloads silently, dirty buffer takes a plain text 3-way merge (`shared/agent-merge` survives the Yjs deletion untouched), unmergeable overlap falls into the same Reconcile path as a git conflict. Needed regardless of the agent, because auto-pull can land on an open file.
