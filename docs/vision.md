...
3. **The assistant is a peer, not a chatbot.** It runs *in* the vault, edits files like a teammate, and asks in plain terms. It uses its native tools; we don't cage it behind a wall of custom ops.
...
Three things that were on this list have shipped and now have PRDs of their own: **Gmail + Calendar** ([`prd/google-mail-calendar.md`](prd/google-mail-calendar.md)), **branded PDF export via Typst** ([`prd/pdf-export.md`](prd/pdf-export.md)), and **agent-authored in-vault apps** ([`prd/vault-apps.md`](prd/vault-apps.md)). What remains:

- PDF/docx preview in place
- Google Drive mirror for vault documents
- App **widgets embedded in a note** — the apps themselves are built; putting one inside the editor is not, and is deliberately not, so the editor stays lean
